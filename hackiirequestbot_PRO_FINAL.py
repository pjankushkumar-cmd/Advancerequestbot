...
class HealthHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if path == "/api/latest":
            try:
                issue, err = api_fetch()
                payload = {"issue": issue} if issue else {"issue": None}
...
def _issue_value(obj):
    if isinstance(obj, dict):
        for key in ("issueNumber", "issue", "period", "periodNumber", "issueNo", "issueNum", "drawNumber"):
            value = obj.get(key)
            if value is not None and str(value).strip():
                return str(value).strip()
        for value in obj.values():
            found = _issue_value(value)
...
def _latest_record(data):
    if isinstance(data, dict):
        d = data.get("data")
        if isinstance(d, dict) and isinstance(d.get("list"), list) and d["list"]:
            return d["list"][0]
...
def _request_api(url):
    import time
    headers = {
        "User-Agent": "Mozilla/5.0 (Linux; Android 14) AppleWebKit/537.36 Chrome/140.0.0.0 Mobile Safari/537.36",
    ...
    r = requests.get(url, headers=headers, timeout=(5, 12), allow_redirects=True)
...
def api_fetch():
    ...
    for template in API_PROXY_TEMPLATES:
        candidates.append(template.format(url=encoded))
    ...
    for candidate in candidates:
        try:
            ...
            issue = _issue_value(record) or _issue_value(data)
            if not issue:
    ...
    logging.error("All WinGo API sources failed: %s", " | ".join(errors))
